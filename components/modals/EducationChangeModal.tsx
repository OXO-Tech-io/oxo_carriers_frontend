...
export default function EducationChangeModal({ isOpen, onClose, initialData }: EducationChangeModalProps) {
  ...
  const handleFormSubmit: SubmitHandler<EducationFormValues> = async (data) => {
    const after = {
      ...
      dateAwarded: data.isOngoing ? null : data.dateAwarded || null,
    ...
    try {
      ...
      toast.success('Change request submitted', 'Your education record change is now pending HR approval.');
      reset();
      onClose();
    } catch {
    ...
    >
      <form id="education-change-form" onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
        ...
        <div>
          ...
          />
          {errors.dateAwarded && <p className="mt-1 text-xs text-red-500">{errors.dateAwarded.message}</p>}
        </div>

        <div className="flex items-center gap-2">
          <input type="checkbox" {...register('isOngoing')} id="isOngoing" className="h-4 w-4 rounded text-[var(--primary)]" />
          <label htmlFor="isOngoing" className="text-xs font-semibold text-[var(--foreground)]">
            Currently pursuing this qualification
          </label>
        </div>
