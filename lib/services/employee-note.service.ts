...
export const employeeNoteService = {
  create: async (employeeUserId: number, content: string, files: File[]): Promise<EmployeeNote> => {
    const formData = new FormData();
    formData.append('employeeUserId', String(employeeUserId));
  ...
  listForEmployee: async (employeeUserId: number): Promise<EmployeeNote[]> => {
    const res = await api.get<ApiResponse<EmployeeNote[]>>(`/employee-notes/employee/${employeeUserId}`);
    return extractData(res);
  },
