  ...
  if (!canAccess) {
    return (
      <div className="flex items-center justify-center min-h-64">
        <div className="bg-[#F9FAFB] border border-[#E4E7EC] rounded-xl p-6 text-center max-w-md">
          <p className="text-[#101828] font-semibold">Access denied</p>
          <p className="text-[#475467] text-sm mt-1">
            You do not have permission to access vouchers.
          </p>
          {permissionError && (
            <p className="text-[#B42318] text-xs mt-3">{permissionError}</p>
          )}
        </div>
      </div>
    );
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[#E4E7EC]">
            <thead className="bg-[#F9FAFB]">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-[#344054] uppercase">
                  Voucher #
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-[#344054] uppercase">
                  Service Provider
                ...
                <th className="px-6 py-4 text-right text-xs font-bold text-[#344054] uppercase">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-[#E4E7EC]">
              {vouchers.map((v) => (
                <tr key={v.id} className="hover:bg-[#F9FAFB]">
                  ...
                  <td
                    className="px-6 py-4 text-sm text-[#475467] max-w-[200px] truncate"
                    title={v.description || ""}
                  >
                    {v.description || "-"}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 flex-wrap">
                      {canReview &&
                        v.status === VoucherStatus.PENDING_REVIEW && (
                          <button
                            onClick={() => {
                      ...
                      {canResubmit &&
                        v.status === VoucherStatus.INFORMATION_REQUEST && (
                          <button
                      ...
                      {canMarkPaid &&
                        v.status === VoucherStatus.BANK_UPLOAD && (
                          <button
      ...
      <div className="flex flex-wrap gap-4">
        <div>
          <label
            ...
            className="block text-xs font-medium text-[#667085] mb-1"
          >
            Status
          </label>
          <select
            ...
            value={filterStatus}
            onChange={(e) =>
              setFilterStatus(e.target.value as VoucherStatus | "")
            }
